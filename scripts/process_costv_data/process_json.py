...
async def process_json_dump(
    ...
    for item in data:
        ...
        try:
            ...
            video_url = item.get("videosource_g", None)
            cover = item.get("video_cover_big", None)
            author = item.get("anchor_nickname", None)
            avatar = item.get("anchor_avatar", None)
            view = item.get("watch_num", None)
            like = item.get("like_num", None)
            gift = item.get("vest_gift", None)
            created_at = item.get("created_at", None)

            text = "Title:" + title + " Introduction:" + introduction
